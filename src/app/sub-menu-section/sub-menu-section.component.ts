...
export class SubMenuSectionComponent implements OnInit {
  ...
  @ViewChild(RightSectionComponent)
  rightSectionComponent: RightSectionComponent;
  @ViewChild(GameDetailsComponent) gameDetail: GameDetailsComponent;
  @ViewChild(GamesDetailsTempComponent)
  gameDetailData: GamesDetailsTempComponent;
  pageUrl: string;
  sectionId: any;
  bannerArray: any;
          ...
          .subscribe((data: any) => {
            if (data) {
              ...
              if (this.numberStatus) {
                ...
                this.menuHeader.openModalRegister(
    ...
    this.sectionService.GetPageDataByGenre(data).subscribe((response: any) => {
      if (response) {
        ...
        if (this.pageDataByGenre != undefined) {
          ...
          this.itemsToShow = this.pageDataByGenre.slice(
            0,
  ...
  getsubCheck(id) {
    if (!this._localStorageService.GetUserId()) {
      ...
      if (!this.IsAndroidPhone) {
        ...
        this._streamingService.GetgameplayLink(data).subscribe((data: any) => {
          ...
          if (data) {
            if (data.statusDescription.statusCode == 200) {
              ...
              this.spinner.hide();
              this.toastr.errorToastr(
                "Your validity of pack has been expired. Please recharge your account and try again",
                "Error",
                {
                  position: "top-left",
                }
        ...
        this.sectionData = id;
        this.gameDetail.openModal(
          this.gameDetail.gameDetails,
          this.sectionData
        );
      }
    }
  ...
  sortA_Z() {
    this.spinner.show();
    this.itemsToShow = this.itemsToShow.sort((a, b) =>

...
} from "src/app/model/account/api/user-account-detail-request-body-post";
...
@Component({
  ...
  styleUrls: ["./subscritpion.component.css"],
})
export class SubscritpionComponent implements OnInit {
      ...
      this.loginService.getCheckNumberAvail(data).subscribe((data: any) => {
        if (data) {
          ...
          if (this.numberStatus) {
            ...
            // this.loginService.ghanaTempSubscribe(phonenumber,this.selectedPack).subscribe((data: any) => {

            // });
            console.log("before sleep::");
            setTimeout(() => {
              this.loginService
                .ghanaSubscribe(phonenumber, this.selectedPack)
                .subscribe((data: any) => {
                  if (data) {
                    ...
                    if (data.statusCode == "200") {
                        ...
                        .subscribe((data: any) => {
                          if (data) {
                            ...
                            if (data.statusDescription.statusCode == "200") {
                              ...
                              data.statusDescription.statusCode == "803"
                            ) {
                              this.closeModal();
                              ...
                              data.statusDescription.statusCode == "802"
                            ) {
                              this.toastr.errorToastr(
                                data.statusDescription.statusMessage,
                                "Error",
                      ...
                      this.toastr.errorToastr(
                        "Subscription Error try again after sometime.",
                        "Error",
                        {
                          position: "top-left",
                        }
  ...
  selectPack(packIndex: number, packId: number) {
    ...
    //   let number = i + 1;
    //   if (number == packIndex) {
    //     this.productList[i].className = "btn btn-danger w-140";
    //   } else {
    //     this.productList[i].className = "btn btn-default w-140";
    //   }
    // }

...
export class V2SearchContainerComponent implements OnInit {
  ...
  SearchValue(searchTerm: string) {
    ...
    if (this.searchTerm.length >= 1) {
      ...
      if (this.searchCategoryFromUrl === 'home') {
        ...
        if(this.searchBarComponent.searchCategories.length > 1 || this.searchBarComponent.searchCategories.length === 0  )
        {
          this.SearchValueForKyeword(this.searchTerm );
        }
        else
        {
        this.SearchValueWithItemType(this.searchBarComponent.searchCategories);
        }        
  ...
  MoreThanOneSearchCategory(): boolean {
    if (this.searchBarComponent.searchCategories) {
      return this.searchBarComponent.searchCategories.length === 0 || this.searchBarComponent.searchCategories.length > 1 ? true : false;
    } else {
      return true;
    }
  }
  SingleSearchCategory(): boolean {
    if (this.searchBarComponent.searchCategories) {
      return this.searchBarComponent.searchCategories.length === 1 ? true : false;
    } else {
      return false;
    }
  }
